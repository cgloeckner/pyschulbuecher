...
class PlannerXls(object):
    ...
    def __call__(self, classes, advance):
        ...
        # query planner
        planner = db.Book.select(lambda b: b.publisher.name == 'schulintern').first()
...
class ClasssetsPdf(object):
    ...
    def __call__(self, person):
        ...
        if n > 0:
            ...
            self.tex += template(self.content, s=self.s, p=person, lns=lns, threshold=self.threshold, pagebreak=self.page_count % 2 == 0)
    
    def saveToFile(self):
        self.tex += template(self.footer)
        
        # export tex (debug purpose)
        dbg_fname = os.path.join(self.texdir, 'Klassensätze_%s.tex' % self.prefix)
...
class InventoryReport(object):
    ...
    def __call__(self):
        ...
        self.tex += template(self.content, s=self.s, all_bks=all_bks, loan_count=loan_count)
...
class BooklistPdf(object):
    ...
    def __call__(self, grade: int, exclude: set, new_students: bool=False):
        ...
        if num_books > 0:
            ...
            tex += template(self.empty, workbook=False, new_students=new_students)
        # render pure workbooks
        if num_books < len(bks):
            tex += template(self.select, grade=grade, bs=bks, workbook=True, exclude=exclude, new_students=new_students)
        else:
